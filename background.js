/**
 * William Yap Proxy Extension for Google Chrome on Desktop
 * The extension uses either chrome.webRequest (slower) in Chrome Stable or chrome.declarativeWebRequest (faster but without stats) in Chrome Beta/Dev.
 * (c) 2016 William Yap. License: Apache 2.0
 */
...
var authHeader = function() {
	...
	return {
		name: 'Chrome-Proxy',
		value: 'ps=' + timestamp + '-' + Math.floor(Math.random() * 10000000) + '-' + Math.floor(Math.random() * 10000000) + '-' + Math.floor(Math.random() * 10000000) + ', sid=' + MD5(timestamp + authValue + timestamp) + ', b=' + chromeVersion[3] + ', p=' + chromeVersion[4] + ', c=win'
	};
};
...
var setProxy = function() {
	...
	if(chrome.declarativeWebRequest) {
		...
		//Chrome Stable
		//Block ads
		chrome.declarativeWebRequest.onBeforeRequest.addListener(
			onCancel,
			{urls: adblockList},
			['blocking']
		);
		//Add auth header
		chrome.declarativeWebRequest.onBeforeSendHeaders.addListener(
			onAddAuthHeader,
			{urls: ['http://*/*']},
			['requestHeaders', 'blocking']
		);
		//Get response on error
		chrome.declarativeWebRequest.onHeadersReceived.addListener(
			onResponse,
			{urls: ['http://*/*']},
			['responseHeaders']
		);
	...
	chrome.browserAction.setTitle({title: 'Chrome-Proxy: Enabled'});
};
...
var unsetProxy = function(userRequested) {
	...
	if(chrome.declarativeWebRequest) {
		...
		//Chrome Stable
		chrome.declarativeWebRequest.onBeforeRequest.removeListener(onCancel);
		chrome.declarativeWebRequest.onBeforeSendHeaders.removeListener(onAddAuthHeader);
		chrome.declarativeWebRequest.onHeadersReceived.removeListener(onResponse);
	}
	if(userRequested) {
		localStorage.setItem('isSetProxy', '0');
		chrome.browserAction.setIcon({path: 'off.png'});
		chrome.browserAction.setTitle({title: 'Chrome-Proxy: Disabled'});
	} else {
		chrome.browserAction.setIcon({path: 'disabled.png'});
		chrome.browserAction.setTitle({title: 'Chrome-Proxy: Temporarily Disabled'});
	}
};
